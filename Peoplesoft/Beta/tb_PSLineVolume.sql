...
SELECT 
COALESCE(@Facility,l.FacilityID) AS COMPANY,
...
'' as NAME
from Lines l
inner join bluebin.DimLocation dl on COALESCE(@Facility,l.FacilityID) = rtrim(dl.LocationFacility) and l.LocationID = dl.LocationID
inner join bluebin.DimFacility df on COALESCE(@Facility,l.FacilityID) = rtrim(df.FacilityID)
