...
SELECT 
BUSINESS_UNIT as FROM_TO_CMPY,
--case when @Facility is not null or @Facility <> '' then COALESCE(@Facility,BUSINESS_UNIT) else BUSINESS_UNIT end as FROM_TO_CMPY,
case when @Facility is not null or @Facility <> '' then (select PSFacilityName from bluebin.DimFacility where FacilityName = COALESCE(@Facility,BUSINESS_UNIT)) else BUSINESS_UNIT end as FacilityName,
