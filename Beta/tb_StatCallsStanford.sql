...
;
/*tb_StatCalls
if not exists(select * from bluebin.Config where ConfigName = 'StatCallsRQDate')  
BEGIN
insert into bluebin.Config (ConfigName,ConfigValue,Active,LastUpdated,ConfigType,[Description])
select 'StatCallsRQDate','',1,getdate(),'Tableau','Stat Calls Custom RQ Date Default  is blank'
END
GO

if not exists(select * from bluebin.Config where ConfigName = 'StatCallsRQLoc')  
BEGIN
insert into bluebin.Config (ConfigName,ConfigValue,Active,LastUpdated,ConfigType,[Description])
select 'StatCallsRQLoc','',1,getdate(),'Tableau','Stat Calls Custom Location to base on.  Default is blank'
END
GO

if not exists(select * from bluebin.Config where ConfigName = 'StatCallsRQOR')  
BEGIN
insert into bluebin.Config (ConfigName,ConfigValue,Active,LastUpdated,ConfigType,[Description])
select 'StatCallsRQOR','1',1,getdate(),'Tableau','Stat Calls Custom Override on RQDate.  Default is 1'
END
GO
    ...
    (
	 select distinct 
		...
		LEFT JOIN bluebin.DimLocation dl on r.REQ_LOCATION = dl.LocationID
		 where  r.REQ_LOCATION = @StatCallsRQLoc and a.TRANS_DATE > @StatCallsRQDate --'2017-07-26 00:00:00.000'  
		 ) a
