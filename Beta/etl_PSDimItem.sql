...
SELECT Row_number()
       ...
       ''							AS PackageString,--****
	   ISNULL(bu.Expireable,'') as Expireable
...
FROM   dbo.MASTER_ITEM_TBL a
	   ...
	   left join (select INV_ITEM_ID,max(ITEM_FIELD_C2) as Expireable from BU_ITEMS_INV where ITEM_FIELD_C2 = 'Y') bu on a.INV_ITEM_ID = bu.INV_ITEM_ID
